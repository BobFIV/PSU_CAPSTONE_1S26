...
if [ -n "$hostIP" ]; then
	docker run -e COLUMNS="`tput cols`" -e LINES="`tput lines`" -it -p 8080:8080 --add-host="localhost:$hostIP" --rm --name acme_in $DOCKERHUBID/acme-onem2m-cse
else
	docker run -e COLUMNS="`tput cols`" -e LINES="`tput lines`" -it -p 8080:8080  --rm --name acme_in $DOCKERHUBID/acme-onem2m-cse
fi