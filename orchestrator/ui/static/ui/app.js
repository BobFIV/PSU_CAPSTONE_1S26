document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!modal || !modalTitle || !modalBody || !btnCloseX || !btnClose || !btnBackTopo || !statusEl || !nodeLeftText || !nodeRightText) {
    return;
  }
  ...
  const state = {
    ...
    cse: { name: "", cseId: "", port: "", extra: "", deployedAction: null },
  ...
  function refreshTopologyText() {
    const cseName = (state.cse.name || "MN-CSE").trim();
    const cseId = (state.cse.cseId || "").trim();
    const csePort = (state.cse.port || "").trim();
    const aeName = (state.ae.name || "sample AE").trim();

    let cseLabel = cseName;
    if (cseId) cseLabel += ` (${cseId})`;
    if (csePort) cseLabel += `:${csePort}`;

    nodeLeftText.innerHTML = `<div>rPI</div><div>${escapeHtml(cseLabel)}</div><div>${escapeHtml(aeName)}</div>`;
    nodeRightText.innerHTML = `<div>rPI</div><div>${escapeHtml(cseLabel)}</div><div>${escapeHtml(aeName)}</div>`;
  }
  ...
  function bindAE(root) {
    ...
    name.addEventListener("input", () => { state.ae.name = name.value; refreshTopologyText(); });
  ...
  refreshTopologyText();
  setStatus("Topology view (UI-only)");
});
