...
# Import AE functions
from ae import register_AE, unregister_AE, retrieve_AE, create_container
...
if sys.argv[1]=='IN':
    ...
    application_name = 'orchestratorAE'                         # The name of the application entity
    application_path = cse_url + '/' + application_name         # The path of the application entity
    subscription_name = 'orchestratorSubscription'                        # The name of the subscription
    originator = 'Corchestrator'
...
# Register an AE
if register_AE(cfg, originator, application_name) == False:
    stop_notification_receiver()
    exit()

# Create a <container> resource
if create_container(originator, application_path, container_name)==False:
    stop_notification_receiver()
    exit()
...
if create_subscription(originator, container_path, subscription_name, notificationURIs) == False:
    unregister_AE(cfg, originator, application_name)
    stop_notification_receiver()
    exit()
